...
// QueryPostTags chains the current query on the "postTags" edge.
...
func (_q *PostQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *PostQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *PostQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *PostQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *PostQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *PostQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// the "postTags" edge. The optional arguments are used to configure the query builder of the edge.
...
func (_q *PostQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Post, init func(*Post), assign func(*Post, *Tag)) error {
	...
	byID := make(map[uint64]*Post)
	nids := make(map[uint64]map[*Post]struct{})
...
func (_q *PostQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Post, init func(*Post), assign func(*Post, *Comment)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "postId" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
