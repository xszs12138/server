...
type UserQuery struct {
	...
	withPosts         *PostQuery
	withAdminComments *CommentQuery
...
// QueryAdminComments chains the current query on the "adminComments" edge.
func (_q *UserQuery) QueryAdminComments() *CommentQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAdminComments: _q.withAdminComments.Clone(),
...
// WithAdminComments tells the query-builder to eager-load the nodes that are connected to
// the "adminComments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAdminComments(opts ...func(*CommentQuery)) *UserQuery {
	query := (&CommentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAdminComments = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withAdminComments; query != nil {
		if err := _q.loadAdminComments(ctx, query, nodes,
			func(n *User) { n.Edges.AdminComments = []*Comment{} },
			func(n *User, e *Comment) { n.Edges.AdminComments = append(n.Edges.AdminComments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*User, init func(*User), assign func(*User, *Post)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "authorId" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadAdminComments(ctx context.Context, query *CommentQuery, nodes []*User, init func(*User), assign func(*User, *Comment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(comment.FieldAdminUserId)
	}
	query.Where(predicate.Comment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AdminCommentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.AdminUserId
		if fk == nil {
			return fmt.Errorf(`foreign-key "adminUserId" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "adminUserId" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
